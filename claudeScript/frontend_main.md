...
endpoint를 체크해줘
  ...
  인증 Flow
  1. Frontend (8000)
   form POST → http://localhost:8090/auth/login

2. Auth (8090)
   인증 성공 → 302 redirect
   → http://localhost:8080/portal/main?access_token=eyJ...

3. Backend (8080) /main
   JWT 토큰 검증 (oauth2ResourceServer) → 302 redirect
   → http://localhost:8000/main?access_token=eyJ...

4. Frontend (8000) /main
   URL에서 access_token 꺼내 localStorage 저장 → 메인 화면
